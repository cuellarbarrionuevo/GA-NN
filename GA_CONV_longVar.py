...
def crear_modelo(individuo):
    ...
    #########################################
    model = Sequential()
    model.add(Conv2D(individuo["filtros"][0], kernel_size=individuo["kernel_size"][0], activation='relu', input_shape=(150, 150, 3)))
    model.add(MaxPooling2D(pool_size=individuo["max_pooling"][0]))
    
    for i in range(1, len(individuo["filtros"])):
        model.add(Conv2D(individuo["filtros"][i], kernel_size=individuo["kernel_size"][i], activation='relu'))
...
def evaluar_fitness(individuo, train_gen, val_gen):
    ...
    #print(individuo)
    model.fit(train_gen, epochs=epochs_v, validation_data=val_gen, verbose=0)
    _, accuracy = model.evaluate(val_gen, verbose=0)
    #print(accuracy)
    evaluaciones_cache[clave] = accuracy
    return accuracy
